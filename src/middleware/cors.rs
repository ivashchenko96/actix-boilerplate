...
pub fn cors_middleware(settings: &Settings) -> Cors {
    ...
    for header in allowed_headers.split(',') {
        ...
        match header.to_lowercase().as_str() {
            ...
            "x-request-id" => cors = cors.allowed_headers(vec![http::header::HeaderName::from_static("x-request-id")]),
            "x-requested-with" => cors = cors.allowed_headers(vec![http::header::HeaderName::from_static("x-requested-with")]),
            _ => {
...
mod tests {
    ...
    fn test_cors_middleware_creation() {
        ...
        let cors = cors_middleware(&settings);
